...
function addLibrary(libraryName, libraryLink) {
	browser.storage.local.get("libraries", function(obj) {
		...
		libraries = browser.storage.local.set({
			libraries: libraries
		}, null);
	});
}
	...
	return function(obj) {
		...
		if (libraries[libraryName]) {
			...
			if (libraryLink.indexOf('lib.overdrive.com') >= 0) {
				...
				libraryResultElement.css("background-image", "url('" + browser.extension.getURL('icons/throbber.gif') + "')");
...
$(document).ready(function() {
	$("body").prepend("\
					<style>\
						a.AGselect { \
						background-image: url('" + browser.extension.getURL('icons/icon48.png') + "');\

...
$(document).ready(function() {
	...
	$("#usernav").prepend("<li><a target='_blank' href='" + browser.extension.getURL("src/options/index.html") + "'><img id='AGimg' src='" + browser.extension.getURL('icons/icon25.png') + "' style='width:16px;height:16px' title='Available Goodreads settings'></a></li>");
	$("#AGimg").mouseover(function() { 
            $(this).attr("src", browser.extension.getURL('icons/icon25-hover.png'));
        })
        .mouseout(function() {
            $(this).attr("src", browser.extension.getURL('icons/icon25.png'));
        });

	browser.storage.local.get("showOnPages", function(obj) {
...
browser.runtime.onMessage.addListener(function(message, sender, sendResponse) {
	...
	for (var bookIndex in message.books) {
		...
		if (book.isaudio) {
			audioStr = "<img class=AGaudio src='" + browser.extension.getURL('icons/headphones.svg') + "' height=8px width=8px>";
			audioClass = "Audio";
			newScore = 90;
		} else {
			audioStr = "";

...
var settings = new Store("settings", {
  "librarydomain": 'spl.lib.overdrive.com'
});
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  switch (message.type) {
    case 'FROM_GROD_PAGE':
      // create search url
      url = "http://" + settings.get('librarydomain') + "/BANGSearch.dll?Type=FullText&FullTextField=All&FullTextCriteria=" + message.title + "%20" + message.author
      $.ajax({
        url: url,
        success: parseODResults(message.id, url, sender.tab.id),
...
var Book = function(title, copies, total, waiting, isaudio, url) {
  this.title = title
  this.copies = copies
  this.total = total
...
function parseODResults(id, url, tabid) {
  return function(data, textStatus, jqXHR) {
    ...
    if (data.indexOf("No results were found for your search.") > 0) {
      ...
      $("div.img-and-info-contain", data).each(function(index, value) {
        ...
        books.push(new Book(title, copies, total, waiting, isaudio, url));
      })
    }
